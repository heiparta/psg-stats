/*jslint node: true */
"use strict";

var mongodb = require('mongodb');
var models = require('./models');
var Promise = require('bluebird');
var restify = require('restify');
var _ = require('lodash');

module.exports.create_player = function (req, res, next) {
  var player = new models.Player(req.params);
  player.save()
    .then(function () {
      req.log.info("Added player", player.name);
      return Promise.resolve(player);
    })
    .catch(models.MongoError, function (err) {
      return models.Player.findOne({name: req.params.name});
    })
    .then(function (player) {
      if (!_.isEmpty(req.params.series)) {
        req.log.debug("Adding series to player", player.name);
        return models.Series.findOne({name: req.params.series})
          .then(function (series) {
            if (series === null) {
              return next(new restify.NotFoundError('Series not found'));
            }
            series.player = series;
            if (_.includes(_.map(series.players, function (v) { return String(v); }), player.id)) {
                res.send({code: "success", name: player.name, series: series.name});
                return next();
            }
            req.log.debug("Adding player", player.name, "to series", series.name);
            series.players.push(player.id);
            series.save()
              .then(function () {
                res.send({code: "success", name: player.name, series: series.name});
                return next();
              })
              .catch(function (err) {
                req.log.error(err);
                next(err);
              });
          });
      } else {
        res.send({code: "success", name: player.name});
        return next();
      }
    })
    .catch(function (err) {
      req.log.error(err);
      next(err);
    });
};

module.exports.create_series = function (req, res, next) {
  var series = new models.Series(req.params);
  series.save()
    .then(function () {
      console.log("Added series", series.name);
      res.send({code: "success"});
      return next();
    })
    .catch(next);
};

module.exports.add_game = function (req, res, next) {
  var game,
    gameId,
    seriesName,
    gamePromise;
  if (!_.isEmpty(req.params.game)) {
    req.log.info("Updating old game");
    gameId = req.params.game;
    seriesName = req.params.series;
    req.params = _.omit(req.params, ["game", "series"]);
    gamePromise = models.Game.findOne({id: req.params.game});
  } else {
    gamePromise = Promise.resolve();
  }
  gamePromise
    .then(function (result) {
      if (gameId && !result) {
        return next(new restify.NotFoundError("Game not found"));
      } else {
        game = result || new models.Game(req.params);
        _.merge(game, req.params);
        req.log.info("Game", game.id);
      }
      return models.Series.findOne({name: seriesName});
    })
    .then(function (series) {
      if (series === null) {
        return next(new restify.NotFoundError('Series not found'));
      }
      req.log.debug("Found series", series.name);
      game.series = series.id;
      return game.save();
    })
    .then(function () {
      req.log.info("Added game", game.date);
      res.send({code: "success"});
      return next();
    })
    .catch(function (err) {
      req.log.error(err);
      next(err);
    });

};
