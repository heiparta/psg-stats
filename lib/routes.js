...
module.exports.create_player = function (req, res, next) {
  var player = new models.Player(req.params);
  player.save()
    .then(function () {
    ...
    .then(function (player) {
      if (!_.isEmpty(req.params.series)) {
        ...
        return models.Series.findOne({name: req.params.series})
          .then(function (series) {
            ...
            if (_.includes(_.map(series.players, function (v) { return String(v); }), player.id)) {
                res.send({code: "success", name: player.name, series: series.name});
                return next();
            }
        ...
        res.send({code: "success", name: player.name});
        return next();
      }
    })
    .catch(function (err) {
...
module.exports.get_game = function (req, res, next) {
  models.Game.findOne({id: models.Id(req.params.game)})
    .populate('series playersAway playersHome')
    .then(function (game) {
      if (game === null) {
        throw new restify.NotFoundError("Game not found");
      }
...
module.exports.add_game = function (req, res, next) {
  ...
  gamePromise
    ...
    .then(function (series) {
      ...
      req.log.debug("Found series", series.name);
      game.series = series.id;
      if (_.isEmpty(playersAway)) {
        return Promise.resolve([]);
      } else {
        return models.Player.where('name').in(playersAway);
      }
    })
    .then(function (players) {
      var missing = _.difference(playersAway, players);
      if (_.isEmpty(playersAway) && !_.isEmpty(missing)) {
        throw new restify.NotFoundError("Player(s) not found: " + missing);
      }
      _.forEach(players, function (player) {
        game.playersAway.push(player.id);
      });
      if (_.isEmpty(playersHome)) {
        return Promise.resolve([]);
      } else {
        return models.Player.where('name').in(playersHome);
      }
    })
    .then(function (players) {
      var missing = _.difference(playersHome, players);
      if (_.isEmpty(playersHome) && !_.isEmpty(missing)) {
        throw new restify.NotFoundError("Player(s) not found: " + missing);
      }
      _.forEach(players, function (player) {
        game.playersHome.push(player.id);
      });
      return game.save();
    })
    .then(function () {
